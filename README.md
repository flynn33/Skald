...
| `xlsx`, `ods` | `WorkbookConverter` | Bounded ZIP/XML readers preserve ordered worksheets, sparse typed cells, source numeric lexemes, formulas with unverified caches, floating dates/epoch, and merged ranges; variant limits are documented. |
| `xls` | `WorkbookConverter` | Real compound-file/BIFF8 worksheet subset; binary numbers retain round-trip precision, caches are unverified, and unsupported variants fail explicitly. |
| `zip` | `ZipCollectionConverter` | ZIP32 stored/deflated collection with safe path and expansion checks, registry-routed members, nested provenance, and visible unknown-member outcomes. |
...
JSON is emitted as a readable document model (version `1.1` for unchanged formats; version `2.0` for CSV/TSV, PDF, images, XML, workbooks, and ZIP collections) with:
...
Workbook schema `2.0` uses ordered top-level `sheets[]` with typed sparse `cells[]`, formula/cache/date fields, and merged ranges. ZIP schema `2.0` uses ordered `members[]` with archive paths, depth, status, and per-member converted content. These shapes are described in [named-format boundaries](docs/remediation/NAMED_FORMAT_READERS.md).
...
- Unknown extensions with confirmed strict UTF-8 text use the text converter; unknown binary content is skipped with a reason. XLS supports the documented BIFF8 worksheet subset, and ZIP supports bounded ZIP32 stored/deflated members; other variants fail with a reason.
